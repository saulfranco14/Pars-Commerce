-- Sync sales_commissions when assigned_to changes on a paid order

CREATE OR REPLACE FUNCTION sync_assignment_on_paid_order()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  IF NEW.status = 'paid' AND NEW.assigned_to IS NOT NULL AND (OLD.assigned_to IS DISTINCT FROM NEW.assigned_to) THEN
    ...
    INSERT INTO public.sales_commissions (
      ...
      NEW.tenant_id, NEW.id, NEW.assigned_to,
...
CREATE TRIGGER order_assignment_sync_trigger
  AFTER UPDATE OF assigned_to ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION sync_assignment_on_paid_order();
