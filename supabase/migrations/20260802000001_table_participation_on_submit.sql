-- A table QR is a menu entry point, not a reservation.  A customer becomes a
-- participant only when they submit at least one item.  Keeping this work in
-- one transaction avoids two simultaneous first orders creating two accounts
-- for the same table.
...
BEGIN
  ...
  -- The lock serializes the first real submission for this QR.
  SELECT * INTO v_qr
  ...
  v_is_new_contributor := v_device_id IS NULL OR NOT EXISTS (
    ...
    WHERE order_id = v_order_id AND added_by_device_id = v_device_id
  );
  ...
  SET status = 'in_progress',
      ...
      diner_count = (
        SELECT count(*) FROM public.order_devices d
        WHERE d.order_id = v_order_id
          AND EXISTS (
            SELECT 1 FROM public.order_items i
            WHERE i.order_id = v_order_id AND i.added_by_device_id = d.id
...
-- Old versions claimed account ownership before a customer named themselves
-- or submitted a product.  Release only those empty anonymous placeholders;
-- real customers and historical payments are deliberately untouched.
UPDATE public.order_devices d
SET is_owner = false,
    updated_at = now()
WHERE d.is_owner = true
  AND nullif(trim(d.display_name), '') IS NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.order_items i
    WHERE i.order_id = d.order_id
      AND i.added_by_device_id = d.id
  );

-- A draft with no lines was created by a scan, never by a purchase.  Releasing
-- its QR makes the table available again without deleting audit data.
UPDATE public.qr_codes qr
SET current_order_id = NULL,
    updated_at = now()
WHERE qr.kind = 'table'
  AND qr.current_order_id IS NOT NULL
  AND EXISTS (
    SELECT 1 FROM public.orders o
    WHERE o.id = qr.current_order_id
      AND o.status = 'draft'
