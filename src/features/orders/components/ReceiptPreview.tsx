...
function formatAddressLine(addr: TenantAddress): string[] {
  ...
  const postalCountry = [addr.postal_code, addr.country]
    .filter(Boolean)
    .join(", ");
  ...
  return (
    <div className="receipt-ticket relative mx-auto w-[302px] max-w-full font-sans text-foreground print:w-[302px] print:max-w-[302px]">
      {isPaid && (
        ...
        >
          <div
            className="select-none text-6xl font-bold text-emerald-500/25 -rotate-[-25deg] whitespace-nowrap"
      ...
      <div className="relative z-10">
        ...
        {shouldShow(opts.showCustomerInfo) &&
          (order.customer_name ||
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b-2 border-foreground">
                  <th className="pb-2 pt-1 text-left text-xs font-semibold">
                    Item
                  </th>
                  <th className="w-12 pb-2 pt-1 pr-2 text-right text-xs font-semibold">
                    Cant.
                  </th>
                  <th className="w-16 pb-2 pt-1 pr-2 text-right text-xs font-semibold">
                    P. unit.
                  </th>
                  <th className="w-16 pb-2 pt-1 text-right text-xs font-semibold">
                    Subtotal
                  </th>
                </tr>
              </thead>
        ...
        <div className="mt-5">
          ...
          <div className="space-y-1.5">
            ...
            {shouldShow(opts.showWholesaleSavings) &&
        ...
        {shouldShow(opts.showPaymentMethod) && order.payment_method && (
          <div className="mt-4 flex items-center justify-center gap-2 rounded border border-border px-3 py-2 print:border-border">
            {PaymentIcon && (
              <PaymentIcon className="h-4 w-4 shrink-0" aria-hidden />
            )}
