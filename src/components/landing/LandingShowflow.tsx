import { ArrowRight, ExternalLink, Tag, ShoppingBag, Percent, Eye } from "lucide-react";

function MockProductList() {
  const products = [
    { name: "Laptop Pro 15\"", price: "$899.00", stock: 12, img: "bg-blue-500/10" },
    { name: "Audifonos BT Max", price: "$45.00", stock: 38, img: "bg-emerald-500/10" },
    { name: "Teclado Mecanico", price: "$120.00", stock: 5, img: "bg-amber-500/10" },
  ];
    ...
    <div className="rounded-xl border border-border bg-surface shadow-card overflow-hidden">
      <div className="flex items-center justify-between border-b border-border bg-surface-raised px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-foreground">Mis productos</span>
        </div>
      ...
      <div className="divide-y divide-border">
        {products.map((p) => (
          <div key={p.name} className="flex items-center gap-3 px-4 py-2.5">
            <div className={`h-9 w-9 shrink-0 rounded-lg ${p.img} flex items-center justify-center`}>
              <ShoppingBag className="h-4 w-4 text-muted-foreground/50" aria-hidden />
            </div>
    ...
    <div className="rounded-xl border border-border bg-surface shadow-card overflow-hidden">
      <div className="flex items-center justify-between border-b border-border bg-surface-raised px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-foreground">Promociones activas</span>
        </div>
      </div>
      <div className="p-4 space-y-2.5">
        <div className="flex items-center gap-3 rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-3 py-2.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-xs font-medium text-foreground">Verano 2026</div>
            <div className="text-[10px] text-muted-foreground">20% en toda la tienda</div>
          </div>
        ...
        <div className="flex items-center gap-3 rounded-lg border border-border px-3 py-2.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-xs font-medium text-foreground">2x1 Audifonos</div>
            <div className="text-[10px] text-muted-foreground">Compra 2, paga 1</div>
          </div>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1 rounded-full bg-accent/10 px-2 py-0.5">
            ...
            <span className="text-[10px] font-medium text-accent">142 visitas</span>
          </div>
        </div>
        {/* Promo banner */}
        <div className="rounded-lg bg-gradient-to-r from-accent/15 to-accent/5 p-2.5 mb-3">
          <div className="text-[10px] font-bold text-accent">Verano 2026 — 20% OFF</div>
          <div className="text-[9px] text-muted-foreground">En toda la tienda</div>
        </div>
        {/* Product grid */}
        <div className="grid grid-cols-2 gap-2">
          {[
            { name: "Laptop Pro", price: "$899", sale: "$719", color: "bg-blue-500/10" },
            { name: "Audifonos BT", price: "$45", sale: "$36", color: "bg-emerald-500/10" },
          ].map((p) => (
            <div key={p.name} className="rounded-lg border border-border p-2">
              <div className={`aspect-square rounded-md ${p.color} mb-1.5 flex items-center justify-center`}>
                <ShoppingBag className="h-5 w-5 text-muted-foreground/30" aria-hidden />
              </div>
              <div className="text-[10px] font-medium text-foreground truncate">{p.name}</div>
              <div className="flex items-center gap-1">
                <span className="text-[10px] font-bold text-accent">{p.sale}</span>
        ...
        <div className="mt-3 flex items-center justify-center gap-1.5 rounded-lg bg-accent py-2">
          ...
          <span className="text-[10px] font-semibold text-accent-foreground">Ver carrito (2)</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-center py-4 lg:py-0">
      <div className="flex h-10 w-10 items-center justify-center rounded-full border border-border bg-surface shadow-soft">
        <ArrowRight className="h-4 w-4 text-accent rotate-90 lg:rotate-0" aria-hidden />
      </div>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
            Da de alta tus productos y promociones desde el dashboard. Tu tienda se actualiza al
            instante y tus clientes compran solo visitando tu URL.
          </p>
        </div>
        ...
        <div className="mt-14 grid items-start gap-0 lg:grid-cols-[1fr_auto_1fr_auto_1fr]">
          ...
          <div>
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-foreground">Administra desde tu dashboard</span>
            </div>
          ...
          <div>
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-foreground">Comparte tu enlace</span>
            </div>
            <div className="rounded-xl border border-border bg-surface p-6 text-center shadow-card">
              ...
              <div className="mt-3 inline-flex items-center gap-2 rounded-lg border border-accent/20 bg-accent/5 px-4 py-2.5">
                <span className="text-sm font-mono font-medium text-accent">tunegocio.pars.com</span>
              </div>
              <p className="mt-4 text-xs text-muted-foreground leading-relaxed">
                Compartelo por WhatsApp, redes sociales, tarjeta de presentacion o donde quieras.
                Tus clientes ven productos y promociones actualizados en tiempo real.
              </p>
              <div className="mt-5 flex flex-wrap justify-center gap-2">
                {["WhatsApp", "Instagram", "Facebook", "Link directo"].map((ch) => (
                  <span key={ch} className="rounded-full border border-border bg-surface-raised px-3 py-1 text-[11px] font-medium text-muted-foreground">
                    {ch}
                  </span>
                ))}
              </div>
          ...
          <div>
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-foreground">Tus clientes compran</span>
            </div>
            <MockStorefront />
          </div>
        </div>
