...
export default function DashboardPage() {
  ...
  const ordersKey =
    activeTenant?.id != null
      ...
      : null;
  const { data: ordersData, isLoading: salesLoading } = useSWR<OrderListItem[]>(
    ordersKey,
    swrFetcher,
    { fallbackData: [] },
  );
            ...
            <TableWrapper>
              ...
              {totalSold > 0 && userSales.length > 0 && (
                <div className="border-t border-border-soft px-4 py-2 text-xs text-muted sm:px-6">
                  ...
                  con {((userSales[0].total / totalSold) * 100).toFixed(0)}% del
                  total.
                </div>
              )}
            </TableWrapper>
                        ...
                        <tbody>
                          {salesByItem.products.map((row, i) => (
                            ...
                            >
                              ...
                              <td
                                className={`${tableBodyCellMutedClass} text-right`}
                              >
                                {row.quantity}
                              </td>
                              <td className={tableBodyCellRightClass}>
                    ...
                    <tbody>
                      {recent.map((o) => {
                        ...
                        return (
                          ...
                          >
                            <td
                              className={`whitespace-nowrap ${tableBodyCellMutedClass}`}
                            >
