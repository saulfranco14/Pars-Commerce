...
export default function CotizacionesPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-5"><PageHeader eyebrow="Ventas" title="Cotizaciones" description="Ofrece, comparte y convierte una cotización en una orden sin perder el seguimiento." action={<button type="button" onClick={() => setOpen(true)} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-accent px-4 text-sm font-bold text-accent-foreground"><Plus className="h-4 w-4" />Nueva cotización</button>} />
    ...
    {isLoading ? <LoadingBlock message="Cargando cotizaciones…" variant="skeleton" /> : quotes?.length ? <div className="grid gap-3 sm:grid-cols-2">{quotes.map((quote) => <article key={quote.id} className="rounded-2xl border border-border bg-surface p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs text-muted-foreground">{quote.quote_number}</p><h2 className="mt-1 font-bold text-foreground">{quote.customer?.name ?? "Cliente"}</h2></div><span className="rounded-full bg-accent/10 px-2.5 py-1 text-xs font-bold text-accent">{STATUS[quote.status] ?? quote.status}</span></div><p className="mt-3 text-2xl font-bold tabular-nums text-foreground">${Number(quote.total).toFixed(2)}</p><p className="mt-1 text-xs text-muted-foreground">Vigente hasta {new Date(quote.valid_until).toLocaleDateString("es-MX")}</p><div className="mt-4 grid grid-cols-2 gap-2">{quote.status === "draft" && <button type="button" onClick={() => void action(quote.id, "review")} className="col-span-2 inline-flex min-h-11 items-center justify-center rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-foreground">Enviar a revisión interna</button>}{quote.status === "internal_review" && <button type="button" onClick={() => void action(quote.id, "ready")} className="col-span-2 inline-flex min-h-11 items-center justify-center rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-foreground">Marcar lista para enviar</button>}{["ready_to_send", "sent", "viewed"].includes(quote.status) && <button type="button" onClick={() => void action(quote.id, "share")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-accent px-3 text-sm font-bold text-accent-foreground"><Send className="h-4 w-4" />Compartir</button>}{["sent", "viewed", "ready_to_send"].includes(quote.status) && <button type="button" onClick={() => void action(quote.id, "accept")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-foreground"><ShoppingCart className="h-4 w-4" />Convertir</button>}{["sent", "viewed"].includes(quote.status) && <button type="button" onClick={() => void action(quote.id, "changes")} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-foreground">Registrar solicitud de cambio</button>}{["sent", "viewed"].includes(quote.status) && <button type="button" onClick={() => void action(quote.id, "reject")} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-destructive/30 bg-surface px-3 text-sm font-semibold text-destructive">Rechazar</button>}{["changes_requested", "conversion_blocked"].includes(quote.status) && <button type="button" onClick={() => void createRevision(quote.id)} className="col-span-2 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-foreground"><Undo2 className="h-4 w-4" />Crear nueva versión</button>}</div></article>)}</div> : <div className="rounded-2xl border border-dashed border-border bg-surface p-8 text-center"><FileText className="mx-auto h-8 w-8 text-accent" /><h2 className="mt-3 font-bold text-foreground">Aún no hay cotizaciones</h2><p className="mt-1 text-sm text-muted-foreground">Crea una para compartir precios y convertirla en orden cuando el cliente acepte.</p></div>}
