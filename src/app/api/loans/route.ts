...
export async function GET(request: Request) {
  ...
  if (!tenantId) {
    return NextResponse.json(
      { error: "tenant_id o loan_id es requerido" },
      { status: 400 },
    );
  }
...
export async function POST(request: Request) {
  ...
  if (payment_plan_type) {
    ...
    insertData.payment_plan_installment_amount =
      payment_plan_installment_amount;
...
export async function PUT(request: Request) {
  ...
  if (body.mp_fee_absorbed_by !== undefined)
    updates.mp_fee_absorbed_by = body.mp_fee_absorbed_by;
  if (body.payment_plan_status !== undefined)
    updates.payment_plan_status = body.payment_plan_status;
  if (body.mp_preapproval_id !== undefined)
    updates.mp_preapproval_id = body.mp_preapproval_id;
  if (body.mp_preapproval_plan_id !== undefined)
    updates.mp_preapproval_plan_id = body.mp_preapproval_plan_id;
  if (body.mp_subscription_init_point !== undefined)
    updates.mp_subscription_init_point = body.mp_subscription_init_point;
  if (body.last_payment_link !== undefined)
    updates.last_payment_link = body.last_payment_link;
  if (body.last_mp_preference_id !== undefined)
    updates.last_mp_preference_id = body.last_mp_preference_id;
